## GraphViz is a powerful graph visualizer graph specifically for visualizing searches in a given search space. It
...
# of the GraphViz class, and load it with the same map file as your search class; the new GraphViz is stored in an
...
# search, it simply calls a method on the attached GraphViz to visually show the evolving search on the graphical map.
...
##  Don't really need to call these to use GraphViz.  These methods are called by the External Interface
...
# x=GraphViz()
# x.loadGraphFromFile("30Node.txt")
